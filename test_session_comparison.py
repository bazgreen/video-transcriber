...
def test_session_comparison():
    ...
    if response.status_code == 200:
        ...
        print(f"   - Comparison features found: {len(found_features)}/{len(comparison_features)}")
    ...
    for session_id in session_ids:
        ...
        if response.status_code == 200:
            ...
            print(f"   ❌ Failed to load insights for {session_id}: {response.status_code}")
    ...
    if comparison_data:
        ...
        if comparison_data['sentiment_trends']:
            ...
            for trend in comparison_data['sentiment_trends']:
                print(f"      {trend['session_name']}: {trend['polarity']:.2f} ({trend['interpretation']})")
        
        # Test topic evolution
        if comparison_data['topic_evolution']:
            print(f"\n   🏷️  Topic Evolution Analysis:")
            print(f"      Common topics across sessions: {len([k for k, v in comparison_data['topic_evolution'].items() if len(v) > 1])}")
            print(f"      Unique topics: {len([k for k, v in comparison_data['topic_evolution'].items() if len(v) == 1])}")
        
        # Test insights patterns
        if comparison_data['insights_patterns']:
            print(f"\n   💡 Key Insights Patterns:")
            total_insights = sum(p['total_insights'] for p in comparison_data['insights_patterns'])
    ...
    if chart_data:
        ...
        if 'sentiment_chart' in chart_data:
            print(f"   - Sentiment chart datasets: {len(chart_data['sentiment_chart']['datasets'])}")
        if 'topic_chart' in chart_data:
            print(f"   - Topic chart labels: {len(chart_data['topic_chart']['labels'])}")
...
def analyze_comparison_data(session_insights):
    ...
    try:
        ...
        for session_id, data in session_insights.items():
            ...
            # Sentiment analysis
            if 'sentiment_analysis' in insights and insights['sentiment_analysis'].get('overall'):
            ...
            # Topic modeling
            if 'topic_modeling' in insights and insights['topic_modeling'].get('main_topics'):
                for topic in insights['topic_modeling']['main_topics']:
                    topic_key = topic.get('description', f"Topic {topic.get('topic_id', 'Unknown')}")
...
def prepare_chart_data(comparison_data):
    ...
    try:
        ...
        if comparison_data['topic_evolution']:
            common_topics = {k: v for k, v in comparison_data['topic_evolution'].items() if len(v) > 1}
            if common_topics:
                ...
                for i, session_name in enumerate(sessions):
                    ...
                    for topic_name in chart_data['topic_chart']['labels']:
                        ...
                        session_topic = next((t for t in topic_sessions if t['session_name'] == session_name), None)
                        dataset['data'].append(session_topic['strength'] * 100 if session_topic else 0)
    ...
    try:
        export_data = {
            ...
            'comparison_summary': {
                ...
                'sentiment_analysis_available': len(comparison_data['sentiment_trends']) > 0,
                'topic_analysis_available': len(comparison_data['topic_evolution']) > 0,
                'insights_analysis_available': len(comparison_data['insights_patterns']) > 0
            },
        ...
        if comparison_data['sentiment_trends']:
            ...
            export_data['comparison_summary']['sentiment_stats'] = {
                ...
                'most_positive_session': max(comparison_data['sentiment_trends'], key=lambda x: x['polarity'])['session_name'],
                'most_negative_session': min(comparison_data['sentiment_trends'], key=lambda x: x['polarity'])['session_name']
            }
        
        if comparison_data['topic_evolution']:
            export_data['comparison_summary']['topic_stats'] = {
                'total_unique_topics': len(comparison_data['topic_evolution']),
                'common_topics': len([k for k, v in comparison_data['topic_evolution'].items() if len(v) > 1]),
                'session_specific_topics': len([k for k, v in comparison_data['topic_evolution'].items() if len(v) == 1])
            }
