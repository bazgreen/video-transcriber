...
from src.config import MemoryConfig, PerformanceConfig, Constants
...
class MemoryManager:
    ...
    def __init__(self, max_memory_percent: int = MemoryConfig.DEFAULT_MEMORY_PERCENT_LIMIT) -> None:
        """
    ...
    def get_optimal_workers(
        ...
        if max_workers is None:
            max_workers = min(multiprocessing.cpu_count(), PerformanceConfig.DEFAULT_MAX_WORKERS)
        ...
        available_for_workers_gb = memory_info['system_available_gb'] - MemoryConfig.SYSTEM_MEMORY_RESERVE_GB
    ...
    def get_memory_recommendations(self) -> Dict[str, str]:
        ...
        if memory_info['system_used_percent'] > 90:
            recommendations['critical'] = "System memory critically low - consider stopping other applications"
        elif memory_info['system_used_percent'] > 80:
            recommendations['warning'] = "High memory usage - monitor for performance impact"
        
        if memory_info['system_available_gb'] < 2:
            recommendations['low_memory'] = "Less than 2GB available - processing may be slow"
