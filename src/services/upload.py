...
def process_upload(transcriber, memory_manager, upload_folder: str) -> Tuple[Dict[str, Any], int]:
    ...
    if file_ext not in config.ALLOWED_FILE_EXTENSIONS:
        raise UserFriendlyError(f'Unsupported format: {file_ext}. Supported formats: {list(config.ALLOWED_FILE_EXTENSIONS)}')
