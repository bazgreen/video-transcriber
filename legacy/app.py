...
if not PSUTIL_AVAILABLE:
    logger.warning(
        "psutil not available - memory monitoring will use conservative estimates"
    )
...
class MemoryManager:
    ...
    def __init__(
        self, max_memory_percent: int = memory_config.DEFAULT_MEMORY_PERCENT_LIMIT
    ) -> None:
    ...
    def get_optimal_workers(
        self,
        min_workers: int = performance_config.MIN_WORKERS,
        max_workers: Optional[int] = None,
    ) -> int:
        """Calculate optimal number of workers based on available memory"""
        if max_workers is None:
            max_workers = min(
                multiprocessing.cpu_count(), performance_config.DEFAULT_MAX_WORKERS
            )
        ...
        available_for_workers_gb = (
            memory_info["system_available_gb"] - memory_config.SYSTEM_MEMORY_RESERVE_GB
        )

        # Calculate max workers based on memory
        memory_based_workers = max(
            performance_config.MIN_WORKERS,
...
class ProgressiveFileManager:
    ...
    def cleanup_all(self) -> None:
        ...
        with self.lock:
            ...
            if self.temp_files:
                logger.info(
                    f"Cleaned up {len(self.temp_files)} temp files "
    ...
    def get_cleanup_stats(self):
        ...
        with self.lock:
            ...
            return {
                ...
                "types": {
                    t: len([f for f in self.temp_files if f["type"] == t])
        ...
        if self._model is None:
            with self._model_lock:
                if self._model is None:  # Double-check locking
                    ...
                    logger.info(
                        f"Whisper model loaded. Memory used: {memory_used:.1f}MB "
...
class ProgressTracker:
    ...
    def update_progress(self, session_id, **updates):
        ...
        with self.lock:
            if session_id in self.sessions:
                ...
                self.emit_progress(session_id)
                logger.debug(
                    f"Progress update for {session_id}: {session['current_task']} "
                    f"({session['progress']:.1f}%)"
                )

    def update_chunk_progress(
        self, session_id, chunk_number, chunk_total, task_description=""
    ):
    ...
    def complete_session(
        self, session_id, success=True, message="Processing complete!"
    ):
                ...
                self.sessions[session_id].update(
                    {
                        ...
                        "progress": 100
                        if success
                        else self.sessions[session_id]["progress"],
    ...
    def emit_progress(self, session_id):
        ...
        if session_id in self.sessions:
            try:
                ...
                logger.debug(
                    f"Emitting progress for session {session_id}: {progress_data.get('progress', 0):.1f}% - {progress_data.get('current_task', 'Unknown')}"
                )
...
class UserFriendlyError(Exception):
    ...
    def __init__(self, error_type, details=None):
        ...
        error_info = self.ERROR_SOLUTIONS.get(
            error_type,
            {
...
class VideoTranscriber:
    def __init__(self) -> None:
        ...
        self.chunk_duration = (
            video_config.DEFAULT_CHUNK_DURATION_SECONDS
        )  # Default chunk duration, can be adjusted for performance
...
def upload_file() -> Union[Response, Tuple[Response, int]]:
    ...
    if file_size > 500 * 1024 * 1024:  # 500MB
        raise UserFriendlyError(
            "large_file_size",
            {"file_size_mb": f"{file_size_mb:.1f}", "max_size_mb": "500"},
        )
...
def delete_session(session_id):
    ...
    if not os.path.abspath(session_dir).startswith(
...
def update_performance_settings():
    ...
    try:
        ...
        if "chunk_duration" in data:
            ...
            if (
                video_config.MIN_CHUNK_DURATION_SECONDS
                <= chunk_duration
                <= video_config.MAX_CHUNK_DURATION_SECONDS
            ):
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": f"Chunk duration must be between {video_config.MIN_CHUNK_DURATION_SECONDS} and {video_config.MAX_CHUNK_DURATION_SECONDS} seconds (provided: {chunk_duration})",
                        }
                    ),
                    400,
                )
        ...
        if "max_workers" in data:
            ...
            max_cpu_limit = min(
                multiprocessing.cpu_count(), performance_config.MAX_WORKERS_LIMIT
            )  # Allow up to CPU count or limit, whichever is lower
            if performance_config.MIN_WORKERS <= max_workers <= max_cpu_limit:
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": f"Max workers must be between 1 and {max_cpu_limit} (provided: {max_workers})",
                        }
                    ),
                    400,
                )
        ...
        return (
            jsonify({"success": False, "error": f"Invalid data format: {str(e)}"}),
            400,
        )
    except Exception as e:
        return (
                ...
                if os.path.exists(metadata_path):
                    try:
                        ...
                        performance_metrics = {
                            ...
                            "words_per_minute": (total_words / (processing_time / 60))
                            if processing_time > 0
                            else 0,
                            "chunks_per_minute": (total_chunks / (processing_time / 60))
                            if processing_time > 0
                            else 0,
                        ...
                        logger.warning(
                            f"Error reading metadata for {session_folder}: {e}"
                        )
...
def handle_join_session(data):
    ...
    if session_id:
        ...
        if current_progress:
            logger.debug(
                f"Sending existing progress to client {request.sid}: {current_progress.get('progress', 0):.1f}%"
            )
            emit("progress_update", current_progress)
        else:
            logger.debug(
                f"No existing progress for session {session_id}, sending not_found status"
            )
            emit(
                "session_status",
