...
def test_education_dictionary_exists():
    ...
    assert 'education' in INDUSTRY_DICTIONARIES, "Education dictionary not found in INDUSTRY_DICTIONARIES"
    print("✓ Education dictionary exists in INDUSTRY_DICTIONARIES")
    
    # Check if it's a dictionary
    assert isinstance(INDUSTRY_DICTIONARIES['education'], dict), "Education dictionary is not a dict"
    ...
    assert len(edu_dict) > 100, f"Education dictionary too small: {len(edu_dict)} terms (expected > 100)"
...
def test_term_consistency():
    ...
    assert len(inconsistent_terms) == 0, f"Inconsistent term mappings: {inconsistent_terms}"
...
def test_comprehensive_coverage():
    ...
    for category, sample_terms in categories.items():
        ...
        print(f"✓ {category}: {coverage_percentage:.1f}% coverage ({len(found_terms)}/{len(sample_terms)} terms)")
        
        # Require at least 75% coverage per category
        assert coverage_percentage >= 75, f"Insufficient coverage for {category}: {coverage_percentage:.1f}%"
