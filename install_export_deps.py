...
def main():
    ...
    print()
    print(
        "This script will help you install optional dependencies for "
    ...
    print(
        f"   PDF Support (reportlab):  "
        f"{'✅ Installed' if reportlab_installed else '❌ Not installed'}"
    )
    print(
        f"   DOCX Support (python-docx): "
        f"{'✅ Installed' if docx_installed else '❌ Not installed'}"
    )
    print()
    ...
    if choice == "1":
        ...
        if not docx_installed:
            success &= install_package(
                "python-docx>=1.1.0", "DOCX support (python-docx)"
            )

        if success:
            print("\n🎉 Full export support installed successfully!")
            print(
                "You now have access to all export formats: SRT, VTT, PDF, "
                "DOCX, Enhanced Text, JSON, HTML"
            )
        ...
        print(
            "Core export formats (SRT, VTT, Enhanced Text, JSON, HTML) "
            "are still available"
        )
